...
| 16 | Observability & Deployment | ◐ Code done, VPS validation pending | main | ops — 334 tests green. Verified: alert routing (P1/P2/P3 channels, dedupe, P1 repeat, grouping, recovery notices, Dhaka business-hours windowing), retention purge per §17.3 (redacts SMS text, KEEPS audit trail, per-company, idempotent), credential cleanup (revoke_at grace, prev-secret 7d, idempotency purge), KEK-rotation (re-encrypt + fails-loudly-on-wrong-key + plan covers every encrypted column) — 24 new tests; all infra syntax-validated (compose/Caddy/Prometheus/alert-rules/workflows/shell). Deliverables: prod compose (api×2+worker+admin+pg+redis, Caddy-only ports), Caddyfile (TLS, health-checked LB, admin allowlist), provision.sh, Dockerfiles, deploy.yml (3 guarded failure paths + pre-deploy backup), backup/restore/verify scripts, alert-rules.yml, operations.md + device-offline-playbook.md. **Pending (needs VPS): induce each alert, timed restore drill, zero-downtime measurement, TLS grade, port scan** |
| 17 | Enablement & Go-Live | ◐ Docs+tests done, pilot pending | main | 343 tests green. System-journey suite (9 tests): **cross-tenant isolation attempted and rejected** (B can't read A's order → 404; mismatched key/company → 401; device token can't register or read; server key can't reach admin; B's order not verifiable by A's device), overpay verifies with delta, suspend→reactivate lifecycle, dead-webhook replay (dry-run then real, history preserved), invariants clean after all journeys. Docs: security-review (T1–T13 each mapped to a proving test), integration-guide completed (what-to-collect table, retry schedule, troubleshooting, limitations), error-codes retry guidance, onboarding checklist, client-agreement EN/BN, support playbook, compliance checklist (15 items evidenced), capacity + k6 scripts, go-live checklist. **Pending: VPS deployment/alert induction/restore drill, device matrix, APK decompile + MITM, dependency audit, and the 2-week pilot (the real acceptance test)** |
